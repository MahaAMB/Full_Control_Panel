...
if(isset($_POST["saveBtn"])){
    ...
    $sql = "UPDATE fullcontrolpanel SET Degree = '$RangeVal' WHERE EngineNum = '1'";
    $query = mysqli_query($db_con,$sql);
    $RangeVal = $_POST['engine2'];
    $sql = "UPDATE fullcontrolpanel SET Degree = '$RangeVal' WHERE EngineNum = '2'";
    $query = mysqli_query($db_con,$sql);
    $RangeVal = $_POST['engine3'];
    $sql = "UPDATE fullcontrolpanel SET Degree = '$RangeVal' WHERE EngineNum = '3'";
    $query = mysqli_query($db_con,$sql);
    $RangeVal = $_POST['engine4'];
    $sql = "UPDATE fullcontrolpanel SET Degree = '$RangeVal' WHERE EngineNum = '4'";
    $query = mysqli_query($db_con,$sql);
    $RangeVal = $_POST['engine5'];
    $sql = "UPDATE fullcontrolpanel SET Degree = '$RangeVal' WHERE EngineNum = '5'";
    $query = mysqli_query($db_con,$sql);
    $RangeVal = $_POST['engine6'];
    $sql = "UPDATE fullcontrolpanel SET Degree = '$RangeVal' WHERE EngineNum = '6'";
    $query = mysqli_query($db_con,$sql);
}
...
if(isset($_POST["onBtn"])){
    $sql = "UPDATE fullcontrolpanel SET State = '1' where State = '0'";
    $query = mysqli_query($db_con,$sql);
}

/* Off button is pressed to update the table with the state */
if(isset($_POST["offBtn"])){
        $sql = "UPDATE fullcontrolpanel SET State = '0' where State = '1'";
        $query = mysqli_query($db_con,$sql);
}

/* Store the direction of the robot movement in the DB */
 if (isset($_POST["fwd"])) {
            $sql = "UPDATE fullcontrolpanel SET Direction = 'Robot is moving forward'";
            $query = mysqli_query($db_con, $sql);
        }
            
if (isset($_POST["bwd"])) {
            $sql = "UPDATE fullcontrolpanel SET Direction = 'Robot is moving backward'";
            $query = mysqli_query($db_con, $sql);
        }
            
if (isset($_POST["stop"])) {
            $sql = "UPDATE fullcontrolpanel SET Direction = 'Robot stopped moving'";
            $query = mysqli_query($db_con, $sql);
        }
            
if (isset($_POST["right"])) {
            $sql = "UPDATE fullcontrolpanel SET Direction = 'Robot is moving to the right'";
            $query = mysqli_query($db_con, $sql);
        }

if (isset($_POST["left"])) {
            $sql = "UPDATE fullcontrolpanel SET Direction = 'Robot is moving to the left'";
            $query = mysqli_query($db_con, $sql);
        }
